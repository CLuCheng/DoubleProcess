...
void AgoraVideoSourceEventHandler::onUserJoined(uid_t uid, int elapsed) {
  LOG_INFO("%s, uid: %d, elapsed :%d", __FUNCTION__, uid, elapsed);
  auto iter = m_videoSource.m_mapMuteRemoteAudioStreams.find(uid);
  if (m_videoSource.mute_all_remote_audio) {
    return;
  }
  if (iter == m_videoSource.m_mapMuteRemoteAudioStreams.end()) {
    m_videoSource.m_mapMuteRemoteAudioStreams[uid] =
        m_videoSource.mute_remote_audio;
    m_videoSource.m_rtcEngine->muteRemoteAudioStream(
        uid, m_videoSource.mute_remote_audio);
  } else {
    m_videoSource.m_rtcEngine->muteRemoteAudioStream(uid, iter->second);
  }
}
