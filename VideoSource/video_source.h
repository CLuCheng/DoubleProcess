...
class AgoraVideoSource : public AgoraIpcListener {
 ...
 public:
   ...
   */
  node_error startPreview(const char* cameraId);
  ...
  bool isLocalview(void* view);

  agora::rtc::VIDEO_PROFILE_TYPE getVideoProfile();

 protected:
  bool joinChannel(const char* key, const char* name, const char* chanInfo,
                   agora::rtc::uid_t uid);
  void notifyJoinedChannel(agora::rtc::uid_t uid);
  void notifyLeaveChannel();
  void notifyRequestNewToken();
  void notifyRenderReady();
  void notifyFirstLocalVideoFrame(int width, int height, int elapsed);
  int notifySetCustomRender(bool enable);
 ...
 private:
  ...
  agora::rtc::AVideoDeviceManager* m_videoDeviceManager;
  bool m_initialized;
  std::string m_params;
  bool mute_all_remote_audio = false;
  bool mute_remote_audio = false;
  void* m_localView = nullptr;
  std::unordered_map<agora::rtc::uid_t, bool> m_mapMuteRemoteAudioStreams;
