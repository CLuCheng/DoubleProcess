...
class AgoraVideoSourceTransporter : public IAgoraVideoSourceTransporter {
 public:
  AgoraVideoSourceTransporter(AgoraVideoSource& videoSource, void* view,
                              bool is_local_view);
...
AgoraVideoSourceTransporter::AgoraVideoSourceTransporter(
    AgoraVideoSource& videoSource, void* view, bool is_local_view)
...
int AgoraVideoSourceTransporter::deliverFrame_I420(
  ...
  stride = stride0;
  if (stride & 0xf) {
    stride = ((stride + 15) & (~0xfu));
  }
  int width = videoFrame.width();
  int height = videoFrame.height();
  int destWidth = stride, destHeight = height;
  if (m_is_local_view) {
  ...
  //if (width != stride0 || height != destHeight)
  I420Scale(planeY, stride0, planeU, strideU, planeV, strideV, width, height,
            (uint8*)y, destWidth, (uint8*)u, destWidthU, (uint8*)v, destWidthU,
            destWidth, destHeight, kFilterNone);
  return 0;
}
...
void AgoraVideoSourceTransporter::getVideoSize(VIDEO_PROFILE_TYPE profile,
  ...
  switch (profile) {
    ...
    case VIDEO_PROFILE_LANDSCAPE_240P_4:  // 424x240   15   220
      ...
      height = 240;
      break;
    case VIDEO_PROFILE_LANDSCAPE_360P:
...
IAgoraVideoSourceTransporter* createAgoraVideoSourceTransporter(
    AgoraVideoSource& videoSource, void* view, bool is_local_view) {
  return new AgoraVideoSourceTransporter(videoSource, view, is_local_view);
}
