...
int D3DRender::Init(HWND hwnd, unsigned int nWidth, unsigned int nHeight, bool isYuv) {
	...
	if (isYuv) {
		...
		//lRet = m_pDirect3DDevice->CreateOffscreenPlainSurface(nWidth, nHeight, D3DFMT_R8G8B8, D3DPOOL_DEFAULT, &m_pDirect3DSurfaceRender, NULL);
...
bool D3DRender::Render(const char *buffer) {
	...
	double fx = m_nWidth / (double)m_nHeight;
	int desRcWidth = m_rtViewport.right - m_rtViewport.left;
	int desRcHeight = m_rtViewport.bottom - m_rtViewport.top;
	int new_height = 1 / fx * (desRcWidth);
	RECT rcDest = m_rtViewport;
	if (new_height > desRcHeight) {
		int new_width = fx * (desRcHeight);
		rcDest.left = (desRcWidth - new_width) / 2;
		rcDest.right = rcDest.left + new_width;
	}
	else {
		rcDest.top = (desRcHeight - new_height) / 2;
		rcDest.bottom = rcDest.top + new_height;
	}
	m_pDirect3DDevice->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
	m_pDirect3DDevice->StretchRect(m_pDirect3DSurfaceRender, NULL, pBackBuffer, &rcDest, D3DTEXF_LINEAR);
